...
class Emailer:
    ...
    def __init__(self, alert_to_email, logger, email_subject=None):
        logger.debug(
            "Creating Emailer with Args - alert_to_email: {alert_to_email}, logger: {logger}, email_subject: {email_subject}".format(
                **locals()))
        ...
        if email_subject:
            ...
            self.logger.debug(
                "Email subject was not provided. Using Default value: {}".format(
                    self.email_subject))
        ...
        if self.alert_to_email is None:
            self.logger.warn(
                "alert_to_email value isn't provided. The email will not be able to send alert emails.")

    def send_alert(self, current_host, retry_count, latest_status_message, latest_start_message):
        self.logger.debug(
            "Sending Email with Args - current_host: {current_host}, retry_count: {retry_count}, latest_status_message: {latest_status_message}, latest_start_message: {latest_start_message}".format(
                **locals()))
        if self.alert_to_email:
            try:
                self.email_backend(
                    self.alert_to_email,
                    self.email_subject,
                    self.EMAIL_BODY.format(current_host, retry_count, latest_status_message,
                                           latest_start_message),
                    files=None,
                    dryrun=False
                )
                self.logger.info('Alert Email has been sent')
            except Exception as e:
                self.logger.critical('Failed to Send Email: {} error'.format(e))
        else:
