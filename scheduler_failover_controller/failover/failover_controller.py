...
class FailoverController:
    ...
    # Set to False in the beginning and then set to True as it becomes active
    IS_FAILOVER_CONTROLLER_ACTIVE = False
    # Set to 0 in the beginning and then is incremented
    RETRY_COUNT = 0
    ...
    def __init__(self, configuration, command_runner, metadata_service, emailer, logger):
        logger.debug(
            "Creating CommandRunner with Args - configuration: {configuration}, command_runner: {command_runner}, metadata_service: {metadata_service}, emailer: {emailer}, logger: {logger}".format(**locals()))
    ...
    def poll(self):
        ...
        self.logger.info("Active Scheduler Node: " +
                         str(active_scheduler_node))
        self.logger.info("Last Failover Heartbeat: " + str(last_failover_heartbeat) +
        ...
        if not self.IS_FAILOVER_CONTROLLER_ACTIVE:
            ...
            if active_failover_node is not None:
                self.logger.info(
                    "There already is an active Failover Controller '" + str(active_failover_node) + "'")
                if active_failover_node == self.current_host:
                    ...
                    failover_heartbeat_diff = (
                        current_time - last_failover_heartbeat).seconds
                    self.logger.debug(
                        "Failover Heartbeat Difference: " + str(failover_heartbeat_diff) + " seconds")
                    max_age = self.poll_frequency * 2
                    self.logger.debug(
                        "Failover Heartbeat Max Age: " + str(max_age) + " seconds")
                    if failover_heartbeat_diff > max_age:
                        self.logger.warning("Failover Heartbeat '" + get_datetime_as_str(last_failover_heartbeat) + "' for Active Failover controller '" + str(
                            active_failover_node) + "' is older then max age of " + str(max_age) + " seconds")
                        self.set_this_failover_controller_as_active()
                        active_failover_node = self.metadata_service.get_active_failover_node()
            else:
        ...
        if self.IS_FAILOVER_CONTROLLER_ACTIVE:
            ...
            if active_failover_node != self.current_host:
                ...
                if not self.is_scheduler_running(active_scheduler_node):
                    ...
                    self.logger.info(
                        "Pausing for 2 seconds to allow the Scheduler to Start")
                    time.sleep(2)
                    if not self.is_scheduler_running(active_scheduler_node):
                        self.logger.warning(
                            "Failed to restart Scheduler on Active Scheduler Node '" + str(active_scheduler_node) + "'")
                        self.logger.warning(
                            "Starting to search for a new Active Scheduler Node")
                        is_successful = False
                        for standby_node in self.get_standby_nodes(active_scheduler_node):
                            ...
                            if self.is_scheduler_running(standby_node):
                                ...
                                self.metadata_service.set_active_scheduler_node(
                                    active_scheduler_node)
                                self.logger.warning(
                                    "New Active Scheduler Node is set to '" + active_scheduler_node + "'")
                                break
                        if not is_successful:
                            self.RETRY_COUNT += 1
                            self.logger.error(
                                "Tried to start up a Scheduler on a STANDBY but all failed. Retrying on next polling. retry count: '" + str(self.RETRY_COUNT) + "'")
                        self.logger.warning(
                            "Finished search for a new Active Scheduler Node")
                    else:
                        self.logger.warning(
                            "Confirmed the Scheduler is now running")
                else:
                    self.RETRY_COUNT = 0
                    self.logger.info(
                    ...
                    for standby_node in self.get_standby_nodes(active_scheduler_node):
                        if self.is_scheduler_running(standby_node):
                            self.logger.warning(
                                "There is a Scheduler running on a STANDBY node '" + standby_node + "'. Shutting Down that Scheduler.")
    ...
    def search_for_active_scheduler_node(self):
        ...
        self.logger.info(
            "Starting to search for current Active Scheduler Node...")
        nodes_with_scheduler_running = []
        for node in self.scheduler_nodes_in_cluster:
            if self.is_scheduler_running(node):
                nodes_with_scheduler_running.append(node)
        self.logger.info("Nodes with a scheduler currently running on it: '" +
                         str(nodes_with_scheduler_running) + "'")
        if len(nodes_with_scheduler_running) > 1:
            ...
            self.logger.info(
                "Nodes do not have a Scheduler running on them. Using Default Leader.")
            active_scheduler_node = self.scheduler_nodes_in_cluster[0]

        self.logger.info(
            "Finished searching for Active Scheduler Node: '" + str(active_scheduler_node) + "'")
